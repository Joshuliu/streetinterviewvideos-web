// The task board's sort space. Personal tasks and meetings live in one
// hand-sortable list per day, so tasks.position and calendar_events.position
// are read as a single number line and a drag can drop any row between any
// two others.
//
...
// anything: calls at the top, then tasks. Dragging pulls a row out of its
// band for good, which is the point.
 ...
 * whenever the meeting time is set or changes, never on other edits, so a
 * hand-dragged call keeps its slot until the call itself moves.
 */
